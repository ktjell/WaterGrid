# -*- coding: utf-8 -*-
"""
Created on Thu May  9 14:30:56 2019
...
class dealer():
    def __init__(self,F, n, t, numTrip):
        ...
        for i in range(n):
            ...
            sock.TCPclient(ips.part_addr[i][0], ips.part_addr[i][1], ['triplets' , l])
        
    def distribute_shares(self, name, s):
        for i in range(self.n):
            sock.TCPclient(ips.part_addr[i][0], ips.part_addr[i][1], [name , int(str(s[i]))])
...
pnr = ips.part_addr.index([ipv4, ips.port])
q = que.Queue()
q2 = que.Queue()
q3 = que.Queue()
